...
- *Decisión:*
  ...
  (f) **Especialidad `energetico` en el enjambre** (`specialties.go` + round-robin de `main.go`, ~1/7 de la flota, `maxDesiredLevel` 4): mismo razonamiento que el aguador un eslabón más arriba — la electricidad es insumo de toda la industria y solo un tipo la produce. *(Seguimiento operativo: el `energetico` es un **generalista** del tipo `generacion` y, al escalar de nivel, sus líneas nuevas se van a las térmicas porque son ~1,6× más baratas por kWh — la capacidad renovable acaba siendo un residuo del arranque. Para que el parque hidroeléctrico se dimensione a propósito existe el binario **`bots-hidro/`**, dedicado en exclusiva a `generacion_hidro`; ver `funcionamiento_bots.md` §2 y §5.4. No cambia esta decisión: es un cliente más, sin backend de por medio.)*
- *Consecuencias:*
  ...
  - (−) `diesel`, `gasolina` y `queroseno` siguen sin consumidor (esperan la fase de transporte/combustible en extractivas de la v2), y `uranio` espera una nuclear futura. *(Resuelto por ADR-028: gasolina y diésel pasan a consumo final urbano, el queroseno se retira y el uranio estrena `generacion_nuclear`, con lo que la lista blanca de la frontera (c) pasa a `{agua, carbón, gas natural, uranio}` — el uranio solo lo produce la `mina`, que no consume electricidad, así que la aciclicidad se mantiene sin tocar el test de factibilidad.)*
...
**ADR-026 — Apagado de clientes por quiebra confirmada por el servidor**

- *Contexto:* la quiebra (§10 del diseño) se evaluaba **solo de forma reactiva**: `bankruptcyService.checkAndApply` se invoca desde cuatro transiciones terminales (cancelar/expirar una orden, completar/cancelar un proceso). Un agente puede quedarse sin absolutamente nada —capital 0, inventario 0, sin órdenes, sin procesos— **sin atravesar ninguna de ellas**: simplemente deja de poder actuar. En ese caso nadie evalúa nunca su quiebra. Y aunque se evaluara, el lado cliente no hacía nada: el `bankruptcy_notice` que ya emitía el servidor solo se logueaba en las estrategias Go. El resultado eran **bots zombi**: sesión y WebSocket abiertos, un tick cada 5 s devolviendo `nil`, indefinidamente. A escala de enjambre (10.000 bots) eso es carga de red, CPU y conexiones sin ninguna contrapartida económica, y además falsea el `[RESUMEN]` del runner, que contaba bots *creados*, no vivos.
- *Decisión:* añadir una vía **pull** y una señal terminal en el cliente.
  - (a) **`POST /agents/me/bankruptcy-check`**: el agente pregunta y el servidor **evalúa y aplica**. Es deliberadamente una mutación y no un `GET` de diagnóstico: partir la decisión en "consultar" + "aplicar" abriría una ventana de carrera y dejaría al cliente decidiendo algo que es competencia exclusiva del servidor autoritativo. Comparte cuerpo, condición, locks y efectos con la vía reactiva (`evaluate(tx, agentId, collectAllReasons)` en `bankruptcy-service.ts`); la única diferencia es que devuelve el detalle en vez de un booleano. Antes de evaluar materializa lazy los procesos vencidos del agente, porque uno vencido sin materializar cuenta como `running` y su output aún no está en inventario.
  - (b) La respuesta incluye `reasons[]` (`has_capital`, `has_inventory`, `has_active_orders`, `has_running_processes`, `role_exempt`) cuando **no** hay quiebra. Un cliente necesita distinguir "me queda un proceso en curso, espero" de "no me queda nada". El acumulado de motivos solo se computa en esta vía: la reactiva conserva su cortocircuito para no encarecer el hot path de cancelación/expiración.
  - (c) **`Engine.Bankrupt()`** en el SDK Go, canal que se cierra una sola vez, análogo a `LowCapital()` pero **terminal**. Cuatro disparadores: 422 `insufficient_capital` confirmado, tick con el estado local a cero (comprobado *antes* del early-return por sueño, porque el bot arruinado vive dormido por el backoff), `bankruptcy_notice` por WS, y 403 `agent_bankrupt` en cualquier acción. Sondeo *throttled* a ≥60 s por bot.
  - (d) **El engine señala; el runner apaga.** `Engine.Stop()` espera al `eventDispatcher`, así que llamarlo desde dentro del propio dispatcher sería un deadlock. Es la misma división de responsabilidades que ya usaba `LowCapital()`.
  - (e) El runner retira al bot (modo normal: vigilante por bot; modo rotación: fuera de la rotación **para siempre**, no cesión de turno) y **termina el proceso cuando todos sus bots han quebrado**, porque ya no queda trabajo posible.
  - (f) *(añadido)* **"Para siempre" tiene que sobrevivir al proceso.** La retirada de (e) solo cubre al bot que está corriendo cuando se confirma la quiebra, y la lista de retirados vivía en RAM. Un bot quebrado mientras dormía entre turnos —`checkAndApply` se dispara al expirar una orden o completar un proceso, sin el engine conectado— o cualquier reinicio del runner devolvía las cuentas muertas a la rotación. Su login devuelve 403 `agent_bankrupt`, y `PerformAuth` auto-registraba ante **cualquier** fallo de login, así que la causa real acababa enmascarada tras una tromba de 409 `username_taken`. Dos correcciones: el auto-registro se condiciona al motivo (solo 401 `invalid_credentials`, el único compatible con "la cuenta no existe"; el 403 se propaga como `auth.ErrAgentBankrupt` y los 5xx/timeout como transitorios), y las bajas se persisten en un fichero append-only por runner (`botkit.BankruptStore`, flag `-bankrupt-file`) que se carga al arrancar. El fichero se invalida con la base de datos: `make clean-docker` lo borra.
- *Consecuencias:*
  - (+) Desaparecen los bots zombi: se liberan conexiones WS, sesiones y CPU en cuanto un agente deja de ser viable.
  - (+) La quiebra deja de depender del azar de las transiciones terminales; la condición §10 se aplica siempre que se cumple.
  - (+) El estado del enjambre pasa a ser observable (`Quebrados` en el `[RESUMEN]`), y una corrida que se apaga sola es una señal económica legítima, no un cuelgue.
  - (−) **Un endpoint autenticado que muta y es invocable a voluntad.** Se mitiga con el throttle del cliente y con que el efecto es estrictamente auto-infligido e idempotente (solo puede quebrarse a sí mismo, y solo si ya cumple la condición). Aun así conviene vigilarlo si algún día se abre el registro a terceros.
  - (−) Sin cambios de esquema (`status`/`bankrupt_at` ya existían), pero sí de contrato: `openapi.yaml` gana `BankruptcyCheck`. De paso se corrigió el `403` que documentaba `GET /agents/me` y que contradecía la regla §10.14 (bankrupt no bloquea lecturas).
  - (−) Un bot quebrado **no se reemplaza**: la flota efectiva decrece. Repoblarla es una decisión de política del enjambre (registrar agentes nuevos), no del ciclo de vida del bot, y queda fuera de este ADR.

**ADR-027 — Especialización del enjambre por oficio (`bots-v2`)**

- *Contexto:* en `bots-v1` la unidad de especialización es el **tipo de instalación** (`specialties.go`: cinco conjuntos que particionan los 16 tipos) y el reparto de la flota es round-robin entre las 6 estrategias (`strats[(i-1)%6]`). Dos consecuencias que ningún ajuste de parámetros corrige:
  - (a) **El reparto no se parece al catálogo.** Con 10.000 bots, 1.667 van a las 2 recetas del agua (833 bots/receta) y otros 1.667 a las 113 industriales (14,7 bots/receta): un factor ~57 de desequilibrio, y justo al revés de donde está la complejidad.
  - (b) **Dentro de un tipo, la cobertura es una lotería.** El nivel de la instalación es un presupuesto de concurrencia **compartido** por todas las recetas del tipo (ADR-021), y la reposición de insumos está acotada a `cuota[tipo] < nivel`: un bot con nivel 3 en `componentes` no cubre sus 20 recetas, cubre 3, y cuáles lo decide el `rnd.Perm` de cada tick. Una receta sin ningún productor real no es un mercado fino: es un eslabón que nadie ejecuta nunca, y todo lo que cuelga de él se para para siempre.
  - `bots-hidro` (ADR-024) es la prueba empírica: una especialización **dentro** de `generacion` que no cabía en el modelo y se resolvió duplicando un binario entero. El mismo problema reaparecería con cualquier receta que un generalista abandone por otra más barata del mismo tipo.
- *Decisión:* un binario nuevo, `bots-v2/`, cuya unidad de especialización es el **oficio**: un conjunto de **recetas concretas** declarado como dato en `bots-v2/oficios.yaml`.
  - (a) **Los oficios se escriben a mano** (66 oficios cubriendo las 152 recetas, con nombres económicos: `aguador`, `siderurgico`, `panadero`, `controlista`…) y se validan contra `infra/seed-config.json` en `oficios_test.go`: cada receta tiene oficio, exactamente uno, y del tipo correcto. Es el mismo papel que `catalog-artifacts.test.ts` cumple con los precios base.
  - (b) **Enabler de esquema: `recipe.key`.** Un oficio nombra recetas, y hasta ahora la API no exponía forma de hacerlo (`recipe_id` se regenera en cada seed y `name` es texto de display) — de ahí que `prioridadRenovablePrimero` tuviera que distinguir la hidro de las térmicas por la *presencia de yacimiento* en sus insumos. Se añade `recipe.key` como espejo exacto de `product.key`, con script de backfill (`backfill-recipe-keys.ts`) para no exigir `clean-docker`. Si el servidor no la expone, el oficio **degrada a todas las recetas de sus tipos** (o sea, al comportamiento de v1) y lo avisa.
  - (c) **Composición de flota por cobertura + peso.** Primero `cobertura_minima` bots a **cada** oficio; el resto proporcional al `peso` declarado, que refleja cuántas recetas dependen de sus outputs. Determinista dado (catálogo, escala), y con `--shard i/N` la unión de los shards es exactamente la flota: en v1 cada instancia genera la flota entera con otro namespace, así que dos runners no reparten trabajo, duplican la economía.
  - (d) **Arranque por capas del grafo** en vez de jitter uniforme: el retardo es `capa × segundos_por_capa` más jitter *dentro* de la capa. El agua entra primero y la industria pesada al final, que es el orden en que la cadena puede producir desde inventario cero (ADR-022).
  - (e) **Apagado y freno declarativos.** `apagado: coste_variable` (parar solo cuando el precio ajeno no cubre el coste variable, propio de la productora marginal) y `freno: almacen` (parar por stock acumulado en vez de por precio, con el suelo de venta bloqueado en el coste). Esos dos campos más `insumos_permitidos` son **todo** lo que distinguía a `bots-hidro`, que pasa a ser un oficio.
  - (f) **Adaptación lenta y pivote de oficio.** El margen y el undercut se ajustan según se acumule o se coloque el stock —en v1 se sortean en `Initialize` y no cambian jamás—, y un bot cuyo nicho lleva ~10 min sin dar margen cambia a otro oficio **de una instalación que ya tiene pagada** (coste hundido: el cambio no cuesta capital). Es el complemento natural de ADR-026: si la quiebra retira bots para siempre, la flota solo puede encoger salvo que un bot inviable pueda reconvertirse.
  - (g) `bots-v2` usa un **namespace UUID propio**: puede correr a la vez que `bots-v1` contra la misma economía, que es la única forma honesta de comparar las dos flotas.
- *Consecuencias:*
  - (+) La cobertura del catálogo deja de ser un sorteo: por construcción ninguna receta se queda sin productores.
  - (+) `bots-hidro` queda subsumido (5 campos de YAML en vez de un binario con su propio `config.yaml`, su `go.mod` y su copia de los precios). La misma vía sirve para cualquier especialización futura dentro de un tipo.
  - (+) El reparto de la flota es inspeccionable **antes** de lanzarla (`-dry-run`) y reproducible entre corridas.
  - (+) *(hallazgo, y poda posterior del catálogo)* Los tests destaparon que **30 de las 152 recetas no llegaban a ningún consumo final** siguiendo el grafo hacia adelante, y como las ciudades son la única demanda final y solo compran `final_consumption` (ADR-025), no había forma de venderlas. El test es **transitivo**: el criterio de un solo nivel daba falsos vivos (al ganado bovino lo compra `procesado_carne`, cuya carne procesada no compra nadie). A raíz de esto se podó el catálogo (v3.2): fuera las ramas de lubricantes y celulosa→papel→cartón más `ensamble_panel_pref`, y los genéricos `frutas`/`verduras`/`lacteos` desglosados en nueve productos concretos de consumo final; `conservas` pasó a consumo final enlatando tomate y sal. El catálogo mantiene 152 recetas y 149 productos, el consumo final sube de 38 a 48 y las recetas invendibles bajan a **20**, en 6 oficios que siguen marcados `sin_demanda` con peso 0 (combustibles —deliberado por ADR-022—, ladrillos/asfalto, la cadena cárnica, bebidas y lana).
  - (+) *(segunda poda, catálogo v3.3)* Fuera los 14 «productos» del tipo `construccion` —plantas industriales, refinerías, astilleros, puertos, edificios, almacenes— con sus 14 recetas `constr_*` y el tipo de instalación entero: eran **edificios**, no bienes que se fabriquen, se guarden en un almacén FIFO y se transporten al mercado, y obligaban a las ciudades a comprar aeropuertos. **135 productos, 138 recetas, 16 tipos**; el consumo final baja de 48 a 34. La cascada —cemento (`caliza`→`cemento`→`hormigon`), madera (`troncos`→`tablas`→`madera_tratada`/`contrachapado`) y carpintería (`ventana`, `puerta_industrial`)— se deja **en** el catálogo y se marca en los oficios: `cantero`, `maderero`, `cementero`, `aserrador` y `cerrajero` pasan a `sin_demanda` con peso 0, y desaparecen los tres oficios `constructor_*` (66 → 63). Las recetas invendibles suben a **30 de 138**.
  - (−) **Un catálogo más que mantener a mano.** Al añadir una receta al seed-config hay que darle oficio; el test lo exige, pero es trabajo manual que la vía generada no habría necesitado.
  - (−) **Dos enjambres que mantener** mientras `bots-v1` no se retire. Se acepta a propósito: sin poder correr las dos flotas en paralelo no hay forma de demostrar que v2 es mejor en vez de suponerlo.
  - (−) `recipe.key` es un cambio de esquema y de contrato (`openapi.yaml`). Sin migraciones (ADR-018), una base viva necesita el backfill.
  - (−) El pivote hace que un bot pueda dejar de hacer aquello para lo que se le contó en la composición de la flota: los cupos de `-dry-run` son la foto inicial, no una garantía permanente.

**ADR-028 — Cierre de las industrias sin salida: la vivienda como demanda final de la construcción**

- *Contexto:* la poda de ADR-027 (catálogo v3.3) retiró el tipo `construccion` porque sus «productos» eran edificios, no bienes transportables, pero dejó **en** el catálogo toda la cascada que los alimentaba. Resultado: **30 de 135 productos** sin ninguna ruta hacia un `final_consumption` —cemento, hormigón, ladrillos, asfalto, la madera entera, la cadena cárnica, la lana, el inoxidable, el perfil de aluminio, los combustibles, las resinas, ventanas y puertas— y 11 oficios marcados `sin_demanda` con peso 0. Un productor de esa zona del grafo paga insumos y salario y se queda el lote en el almacén para siempre: es capital quemado, no un mercado ilíquido.
- *Decisión:* cerrar las 30 cadenas en vez de podarlas otra vez, con cuatro mecanismos:
...
- *Consecuencias:*
  ...
  - (+) Cero cambios de backend: seed-config, artefactos regenerados y bots, como ADR-024. Se aplica con `clean-docker` + re-seed (ADR-018).
  - (+) La cadena más profunda del catálogo pasa a ser la vivienda (~7 industrias convergiendo), lo que da al enjambre una capa 7 y un motivo real para que las capas 1-2 escalen.
