...
- *Consecuencias:*
  ...
  - (+) *(hallazgo)* Los tests destaparon que **21 de los 149 productos no tienen ningún comprador** en el catálogo —ni insumo de otra receta ni `final_consumption`—, agrupados en 7 oficios completos (combustibles, lubricantes, la rama celulosa→papel→cartón, ladrillos/asfalto, bebidas, cárnico y lana). Parte es deliberado (ADR-022 pospone el combustible en las extractivas para no cerrar ciclos), pero el resto parece un hueco del catálogo. Van marcados `sin_demanda` con peso 0 y un test que falla si la afirmación deja de ser cierta.
