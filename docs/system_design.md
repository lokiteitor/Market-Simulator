...
- **Alcance v1:** Operaciones de agente y visibilidad de mercado. **Actualización 2026-07:** el frontend incluye además un **panel de administración** para el operador (`frontend/src/pages/admin`, usuario creado por `seed-admin`), fuera del alcance original de este documento, la pantalla de **instalaciones** (`/installations`, ADR-021) y la del **banco central** (`/bank`, §4.6): la ventanilla de convertibilidad (`GET /bank`, `POST /bank/convert`) ya tiene UI de agente. El **admin es también la cuenta personal del operador humano**: su sidebar muestra las dos secciones (Monitoreo + Mercado) y puede comprar instalaciones de cualquier rol, producir, tradear y convertir oro. El panel admin cubre 6 vistas: `/admin` (resumen), `/admin/agents` (con `population`), `/admin/cities` (ingreso circular ADR-020: pendiente por fuente salario/tasa, repartido 24 h vía `GET /admin/cities`, población y % de reparto por ciudad — desde ADR-029 la población es dinámica, así que esa tabla ordenada por ella es el ranking de qué ciudades están creciendo), `/admin/market`, `/admin/production` (recetas filtrables por tipo de instalación + electricidad producida 24 h) y `/admin/bank` (moneda: masa monetaria y fees históricos, emisión neta vs. capacidad). Las **ciudades** (rol `city`, ADR-020) y el propio agente banco no tienen UI propia — los operan bots/sweepers — pero la UI las etiqueta correctamente (badge "Ciudad", toast de ingreso urbano) por si un humano entra con sus credenciales. La **energía** (ADR-024) no requiere pantalla nueva: la electricidad, el tipo de instalación `generacion` y sus recetas emergen del catálogo, y `/catalog` los hace navegables (columna y filtro por tipo de instalación, chip ⚡ en el insumo eléctrico, filtro por categoría).
...
**Event Log:** Tabla técnica pero legible: `Tipo`, `Timestamp`, `Payload` (expandible como JSON formateado). Cubre también los tipos `gold_converted`, `money_issued`, `deposit_depleted`, `city_income_distributed`, `installation_purchased`, `city_consumed` y `city_population_changed`.
...
| `city_consumed` | WS (solo rol `city`) | Sin toast (ocurre cada pocos segundos); invalida `self` e historial: el servidor acaba de DESTRUIR parte del inventario de la ciudad (ADR-029) y el snapshot cacheado ya no vale. |
| `city_population_changed` | WS (solo rol `city`) | Sin toast; invalida `self` e historial. Cambia la población, que escala tanto el ingreso que recibe como lo que se le consume. |
