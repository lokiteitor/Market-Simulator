...
* **`go-sdk/sdk/botkit/`**: Helpers compartidos con `bots-ciudad` y `bots-hidro`, no duplicados aquí: humanización (precios "bonitos", cantidades perturbadas, TTLs variados, cancel/replace), venta a mercado con suelo de coste (`SellAtMarket`), economía de instalaciones (`InstallationBuyAction`) y rendimiento de yacimientos (`EffectiveOutputQtyCent`). `botkit_aliases.go` los re-exporta con los nombres locales; **al tocar uno, editarlo en `botkit`**.
...
* **[`specialties.go`](file:///home/ddelgado/git/lab/world/bots-v1/specialties.go)**: Reparto del catálogo entre bots por TIPO de instalación, no por rol: `aguador` (pozo_agua), `farmer` (campo, granja, bosque), `miner` (mina, cantera, pozo) y `transformer` (los 9 tipos industriales). Los cinco conjuntos particionan los 20 tipos.
