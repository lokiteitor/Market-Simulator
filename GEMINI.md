...
- Roles de agente: los 2 de mercado (`transformer` —**único rol productivo**, ADR-022: extrae y transforma— y `trader`) más `admin` (panel, solo-monitoreo) y `bank` (banco central, sin credenciales); estos dos no son registrables ni cuentan en agregados de mercado (`NON_MARKET_ROLES` en `types/contracts.ts`). **No existe rol `consumer`** (ADR-025): la demanda final es exclusivamente la de las ciudades (`city`), el único rol consumidor con ingreso recurrente.
