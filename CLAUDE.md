...
- Roles de agente: los 2 de mercado (`transformer` —**único rol productivo**, ADR-022: extrae y transforma— y `trader`) más `admin` (panel de monitoreo **y cuenta personal del operador humano**: puede comprar instalaciones de cualquier rol, producir, tradear y usar la ventanilla; se siembra solo vía `seed-admin` con capital financiado por `fundAgentSeedCapital`, está **exento de quiebra** como las ciudades y sigue fuera de los agregados de mercado) y `bank` (banco central, sin credenciales); estos dos no son registrables ni cuentan en agregados de mercado (`NON_MARKET_ROLES` en `types/contracts.ts`). Y `city` (ciudad-consumidor): **sembrada con credenciales y NO registrable** por humanos, pero **sí** cuenta como demanda de mercado — por eso está en `SEEDABLE_MARKET_ROLES` y no en `MARKET_ROLES` (que es la lista *registrable*) ni en `NON_MARKET_ROLES`. **No existe rol `consumer`** (ADR-025): la demanda final es exclusivamente urbana, porque solo las ciudades tienen ingreso recurrente; un consumidor registrable solo gastaba su capital semilla hasta quebrar.
...
El `city-income-sweeper` del Worker reparte lo pendiente entre las ciudades activas **ponderado por `agent.population`** (peso VIVO desde ADR-029: el que cada ciudad se ha ganado creciendo), con reparto exacto al céntimo (`splitIncomeByWeight`: floor + residuo a la ciudad de mayor peso) y notificación WS `city_income` post-commit. Al tocar dinero, recordar la **nueva** invariante de conservación (sin término de salarios):
...
- **Fase de energía v1 (ADR-024)**: la `electricidad` es un producto `intermediate` normal (almacenable, sin cambios de motor) que consumen las 110 recetas industriales (15-20% del coste en metalurgia/materiales/refinería, 8-12% en el resto) y produce el tipo de instalación `generacion`: `generacion_hidro` (solo agua, el respaldo caro que preserva la raíz única), térmicas de carbón/gas y `generacion_nuclear` (uranio, ADR-028) — más baratas que la hidro, pero de yacimiento finito: al agotarse, la hidro pasa a marginal sola. **Frontera dura**: la electricidad NO entra en las extractivas y la generación solo quema combustibles primarios (agua, carbón, gas, uranio), nunca derivados industriales como el diésel — cerraría ciclos; lo protege un test en `catalog-graph.test.ts`. Las ciudades no la compran. Los bots tienen la especialidad dedicada `energetico` (como el aguador: sin generación no produce la industria).
- **Cierre de las industrias sin salida (ADR-028)**: la poda de la construcción dejó 30 de 135 productos sin ninguna ruta a `final_consumption` —capital quemado, porque las ciudades son la única demanda final (ADR-025)—. Se cerraron con la **vivienda** (tipo `constructora`: hormigón + ladrillos + madera + ventanas + puertas + tubería inox + cableado) como sumidero de la construcción, tres industrias nuevas (`carpinteria`, `carniceria`, `textil`), la recategorización a consumo final de lo que ya era de uso final (aceite vegetal, bebidas, gasolina, diésel; el `textiles` va al revés: pasa a intermedio para ser la tela de la ropa) y la nuclear para el uranio. Lo vigila el test **`toda receta llega a consumo final`** de `catalog-graph.test.ts`, la contrapartida hacia adelante del test de huérfanos. Dos trampas al ampliar el catálogo: el coste propagado toma el **mínimo** entre las recetas de un mismo output (una segunda receta más cara nace bajo coste para siempre), y cada `final_consumption` nuevo **diluye** la demanda urbana, que se reparte uniformemente entre todos ellos (ADR-029 lo hace explícito: el presupuesto per cápita se divide entre el tamaño de la cesta).

### Demanda urbana endógena (ADR-029)

La población de las ciudades es una **variable de estado**, no una constante del seed, y lo que compran **se destruye**. Cuatro reglas, todas del servidor, en el `city-consumption-sweeper` (`city-consumption-service.ts`, una tx POR CIUDAD con `SKIP LOCKED` para no solaparse con sus órdenes en vuelo):

1. **Arranque uniforme**: toda ciudad nace con `CITY_INITIAL_POPULATION` (1000 hab) y el mismo `CITY_SEED_CAPITAL_CENTS`. `infra/cities.json` es solo la lista de cuentas; `agent.population_weight` pasó a ser `agent.population` y es **mutable**.
2. **Crecimiento**: la `vivienda` comprada se **absorbe** (el lote se destruye) y suma `CITY_HABITANTS_PER_HOUSING` habitantes por unidad. Cuál es el bien de inversión lo dice `product.urban_role = 'housing'`, no la key.
3. **Decaimiento con suelo**: `CITY_POPULATION_DECAY_BPS_PER_SIM_DAY` por día simulado, recortado en la población inicial. Es lo que da demanda **estructural** a la construcción, y el suelo garantiza que la demanda final nunca se apaga.
4. **Consumo destructivo**: la cesta (`urban_role = 'basket'`) se consume best-effort con `consumeAvailableFifoUpTo` (variante *best-effort* de `consumeAvailableFifo`: el déficit es un dato del modelo, no un 422). El presupuesto `CITY_NEED_BUDGET_PER_CAPITA_CENTS_PER_SIM_HOUR` se reparte entre la cesta **en dinero** y se convierte a unidades con `product.reference_cost_cents` (coste propagado, ahora persistido por el seed).

Dos sutilezas que hay que respetar al tocar esto:

- **El residuo del redondeo se ACUMULA**: la necesidad se calcula como diferencia de dos acumulados enteros sobre `agent.consumed_pop_seconds` (Σ población × Δt simulado), no como un `floor` por pasada. Con truncamiento por pasada, todo bien que cueste más de ~578 ¢ tendría demanda cero **para siempre** y 40 de los 49 finales se quedarían sin salida (lo que ADR-028 vino a cerrar). El acumulador va en habitante-segundos y no en segundos para que un cambio de población no genere un pico retroactivo.
- **La conservación monetaria NO cubre esto** (no se toca dinero): un error de contabilidad de unidades no lo delata `market_conservation_delta_cents`. La vigilancia es `city_consumption_units_total` contra `production_units_total`, más `city_need_unmet_units_total`.

Las tres perillas van **acopladas**: `decay_bps_día = 10000 × hab_por_vivienda × 24 × b × r / coste_vivienda` (con `r` la cuota del presupuesto dedicada a vivienda), y el capital semilla debe cubrir al menos una vivienda o ninguna ciudad crecerá. `GET /agents/me/city-needs` es la vista autoritativa que consumen los bots: la fórmula no se replica en el cliente.

## Bots (`bots-v1/` + `go-sdk/`)

Ver `docs/funcionamiento_bots.md`. Un binario Go lanza N bots como goroutines; cada bot usa el `engine.Engine` del SDK (`go-sdk/sdk/`): auth con re-login automático (los refresh tokens son de un solo uso), snapshot, WS con reconexión, tick periódico. Las estrategias (`producer.go` + `specialties.go`, `trader.go`; la consumidora vive en `botkit` y solo la usan las ciudades) devuelven acciones declarativas; nunca llaman a la API directamente. `bots-v1/config.yaml` contiene los precios base de los 150 productos —**generados**, no escritos a mano: `cd backend && bun src/scripts/generate-catalog-artifacts.ts` los propaga por coste desde `infra/seed-config.json` a `bots-v1/`, `bots-v2/`, `bots-ciudad/`, `bots-hidro/` y las tablas del catálogo— y `sim_time_factor`, que **debe coincidir** con el `SIM_TIME_FACTOR` del backend o todos los cálculos de margen quedan sesgados. El directorio `bot-engine/` fue eliminado; no referenciarlo.
