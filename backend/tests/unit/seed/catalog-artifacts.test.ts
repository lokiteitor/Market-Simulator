 ...
 *   - el bloque `prices:` de `bots-v1/config.yaml`, `bots-v2/config.yaml`,
 *     `bots-ciudad/config.yaml` y `bots-hidro/config.yaml` (el MISMO en los cuatro);
...
describe("artefactos derivados del catálogo", () => {
  test("los cuatro binarios de bots llevan el mismo bloque de precios", async () => {
